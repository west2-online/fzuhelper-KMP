 ...
 * @constructor
 */
class FeedbackRepository(val client: HttpClient, val shareClient: ShareClient) {
  /**
   * 创建新的反馈
   *
          ...
          .submitForm(
            ...
            formParameters =
              Parameters.build {
                ...
                labels.forEach { append("Label", it) }
              },
          )
          .body<FeedbackSubmit>()
      emit(response)
  ...
  fun getFeedbackDetailByGithub(id: Long): Flow<GithubIssue> {
    return flow {
      val response =
        shareClient.client.get("${FuTalkFeedbackQuestionUrl}/issues/${id}").body<GithubIssue>()
      emit(response)
    }
  }
  ...
  fun getFeedbackDetailCommentsByGithub(id: Long): Flow<List<GithubCommentsItem>> {
    return flow {
      val response =
        shareClient.client
          .get("${FuTalkFeedbackQuestionUrl}/issues/${id}/comments")
          .body<List<GithubCommentsItem>>()
      emit(response)
    }
  }
  ...
  fun getGithubIssues(pageInt: Int): Flow<List<GithubIssueByPageItem>> {
    return flow {
      val response =
        shareClient.client
          .get("${FuTalkFeedbackQuestionUrl}/issues") {
