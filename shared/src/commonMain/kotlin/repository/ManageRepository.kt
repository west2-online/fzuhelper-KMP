 ...
 * @constructor
 */
class ManageRepository(val client: HttpClient) {
  /**
   * 处理据举报的帖子
   *
   * @param postId Int
      ...
      val result =
        client
          .submitForm(
            ...
            formParameters = parameters { append("imageName", imageName) },
          )
      ...
      val result =
        client
          .submitForm(
            ...
            formParameters = parameters { append("ribbon", imageName) },
          )
          .body<RibbonDelete>()
      emit(result)
    }
  ...
  fun getUserByEmail(email: String): Flow<UserDataByEmail> {
    return flow {
      val response =
        client
          .get("/manage/user") { url { parameters.append("email", email) } }
  ...
  fun addAdmin(email: String): Flow<AdminAdd> {
    return flow {
      val response =
        client
          .submitForm(url = "/manage/admin") { formData { append("email", email) } }
          .body<AdminAdd>()
      emit(response)
    }
  }
