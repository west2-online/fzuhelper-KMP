...
class FeedBackViewModel(private val feedbackRepository: FeedbackRepository) : ViewModel() {
  ...
  private val _detailResult =
    CMutableStateFlow(MutableStateFlow<NetworkResult<GithubIssue>>(NetworkResult.UnSend()))
            ...
            when (it) {
              ...
              is NetworkResult.Success -> {
                if (it.dataForShow.number != null) {
                  var data: NetworkResult<List<GithubCommentsItem>> =
                    NetworkResult.UnSend<List<GithubCommentsItem>>()
                  feedbackRepository
                    .getFeedbackDetailCommentsByGithub(it.dataForShow.number)
  ...
  val detailComment =
    MutableStateFlow<NetworkResult<List<GithubCommentsItem>>>(NetworkResult.UnSend())
  ...
  fun submitNewFeedback(content: String, title: String, label: List<String>) {
    viewModelScope.launch {
      _submitResult.logicIfNotLoading {
        if (title == "") {
          _submitResult.resetWithLog(
            "submitNewFeedBack/submitNewFeedBack",
            NetworkResult.Error(Throwable("标题不得为空"), Throwable("标题不得为空")),
          )
        }
        if (content == "") {
          _submitResult.resetWithLog(
            "submitNewFeedBack/submitNewFeedBack",
            NetworkResult.Error(Throwable("内容不得为空"), Throwable("内容不得为空")),
          )
        }
        feedbackRepository
          .submitNewFeedBack(content, title, label)
          .actionWithLabel(
            "submitNewFeedBack/submitNewFeedBack",
  ...
  fun postFeedbackDetailComment(content: String, id: Int) {
    viewModelScope.launch {
      _commentResult.logicIfNotLoading {
        feedbackRepository
          .postFeedbackDetailComment(content, id)
          .actionWithLabel(
  ...
  /** Post list flow 反馈列表分页 */
  val feedbackListFlow =
...
data class FeedbackPageData(val result: List<GithubIssueByPageItem>?, val nextPageNumber: Int?)

class EasyFeedbackPageSource(private val backend: LoadFeedBackPageData) :
  PagingSource<Int, GithubIssueByPageItem>() {
  override suspend fun load(params: LoadParams<Int>): LoadResult<Int, GithubIssueByPageItem> {
    return try {
