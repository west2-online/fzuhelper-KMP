...
class LoadPageDataForPost(val getResult: suspend (page: Int) -> List<PostListItemData>?) {
...
data class PageLoadDataForPost(val result: List<PostListItemData>?, val nextPageNumber: Int?)

class EasyPageSourceForPost(private val backend: LoadPageDataForPost) :
  PagingSource<Int, PostListItemData>() {
  override suspend fun load(params: LoadParams<Int>): LoadResult<Int, PostListItemData> {
    return try {
...
class EasyPageSourceForCommentPreview(private val backend: LoadPageDataForCommentPreview) :
  ...
  override suspend fun load(
    ...
    return try {
      ...
      LoadResult.Page(data = response.result!!, prevKey = null, nextKey = response.nextPageNumber)
    } catch (e: Exception) {
      LoadResult.Error(Throwable("加载失败"))
    }
  }
...
class EasyPageSourceForCommentTree(private val backend: LoadPageDataForCommentTree) :
