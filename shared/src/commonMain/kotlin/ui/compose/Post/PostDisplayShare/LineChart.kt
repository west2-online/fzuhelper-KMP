...
fun XYChart(thumbnail: Boolean, modifier: Modifier = Modifier, data: LineChartDataForShow) {
  val colorMap = remember { data.createColorMap() }
  if (data.isAvailable()) {
    Box(
      modifier =
      ...
      ChartLayout(
        ...
        XYGraph(
          ...
          xAxisLabels = { Text(it, Modifier.padding(top = 2.dp)) },
          xAxisTitle = {
            Box(modifier = Modifier.fillMaxWidth(), contentAlignment = Alignment.Center) {
              Text(data.xAxisTitle)
            }
          },
          yAxisLabels = { Text(it.toString(), Modifier.absolutePadding(right = 2.dp)) },
          yAxisTitle = {
          ...
          data.yMap.entries
            ...
            .forEach { (city, rain) ->
              chart(
                city,
                rain.mapIndexed { index, d -> DefaultPoint(data.xData[index], d.toFloat()) },
                thumbnail,
                colorMap = colorMap,
              )
            }
...
private fun XYGraphScope<String, Float>.chart(
  ...
  LinePlot(
    data = data,
    lineStyle = LineStyle(brush = SolidColor(colorMap[city] ?: Color.Black), strokeWidth = 2.dp),
    symbol = { point ->
      Symbol(
        shape = CircleShape,
  ...
  if (!thumbnail) {
    Surface {
      FlowLegend(
        ...
        symbol = { i ->
          Symbol(modifier = Modifier, fillBrush = SolidColor(colorMap[cities[i]] ?: Color.Black))
        },
        label = { i -> Text(cities[i]) },
      )
    }
  }
}
...
class LineChartDataForShow(
  ...
  fun isAvailable(): Boolean =
    xData.size == yMap.map { it.value.size }.max() &&
      xData.size == yMap.map { it.value.size }.min() &&
      xData.isNotEmpty() &&
      yMap.isNotEmpty()
  ...
  fun createColorMap(): Map<String, Color> {
    ...
    return buildMap { yMap.keys.toList().fastForEachIndexed { i, item -> put(item, colors[i]) } }
  }
}
