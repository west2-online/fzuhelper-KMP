...
class AuthenticationViewModel(
  ...
  private val tokenKValueAction: TokenKValueAction,
) : ViewModel() {
  private val _registerCaptcha =
  ...
  private val _studentCaptcha =
    CMutableStateFlow(MutableStateFlow<NetworkResult<ImageBitmap>>(NetworkResult.UnSend()))
  val studentCaptcha = _studentCaptcha.asStateFlow()

  private val _verifyStudentIDState =
    CMutableStateFlow(MutableStateFlow<NetworkResult<TokenData>>(NetworkResult.UnSend()))
  ...
  fun getRegisterCaptcha(email: String) {
    viewModelScope.launch {
      _registerCaptcha.logicIfNotLoading {
        loginRepository
          .getRegisterCaptcha(email = email)
          .actionWithLabel(
            "getRegisterCaptcha/getRegisterCaptcha",
            catchAction = { label, error ->
              _registerCaptcha.resetWithLog(
                label,
                NetworkResult.Error(error, Throwable("申请失败,请稍后重试")),
              )
            },
      ...
      _verifyStudentIDState.logicIfNotLoading {
        loginRepository
          ...
          .actionWithLabel(
            ...
            catchAction = { label, error ->
              _verifyStudentIDState.resetWithLog(
                label,
                networkErrorWithLog(Throwable("登录失败"), "验证失败"),
              )
            },
          )
      }
  ...
  /** Refresh student captcha 刷新教务处的验证码 */
        ...
        loginRepository
          ...
          .actionWithLabel(
            ...
            collectAction = { label, data ->
              ...
              if (data.code == 12) {
                val token = tokenKValueAction.token
                token.setValue(data.data.toString())
