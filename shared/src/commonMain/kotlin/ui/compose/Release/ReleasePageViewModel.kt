...
class ReleasePageViewModel(private val releaseRepository: PostRepository) : ViewModel() {
  private val _newPostState =
    CMutableStateFlow(MutableStateFlow<NetworkResult<String>>(NetworkResult.UnSend()))
  val newPostState = _newPostState.asStateFlow()

  private var _labelList =
    CMutableStateFlow<NetworkResult<List<Label>>>(MutableStateFlow(NetworkResult.UnSend()))
   ...
   */
  fun newPost(releasePageItemList: List<ReleasePageItem>, title: String, labelList: List<Int>) {
      ...
      _newPostState.logicIfNotLoading {
        ...
        if (
          list.filterIsInstance<ReleasePageItem.ImageItem>().any {
            it.let { (it.image.value?.size ?: 0) / 1024 / 8 > 5 }
          }
        ) {
        ...
        releaseRepository
          .newPost(releasePageItemList = list, title = title, labelList = labelList)
          .actionWithLabel(
            "newPost/newPost",
