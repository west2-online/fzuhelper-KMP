...
/**
 * Year opens dao 对可选学年的数据库操作
 *
 * @constructor Create empty Year opens dao
 */
class YearOpensDao() {
  /** Clear all year opens 清除可选学年 */
   ...
   */
  fun insertYearOpens(list: List<String>) {
    database.yearOptionsQueries.transaction {
      clearAllYearOpens()
      list.forEach { database.yearOptionsQueries.insertYearOptions(it) }
    }
  }
